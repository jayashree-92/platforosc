    ...
    public class OutboundSwiftMsgCreator
    {
        ...
        private static Field32A GetField32A(WireTicket wire)
        {
            var f32A = new Field32A()
                .setDate(wire.HMWire.ValueDate)
                .setCurrency(wire.IsFundTransfer || wire.IsNoticeToFund ? wire.ReceivingAccount.Currency : wire.SSITemplate.Currency)
        ...
        private static void SetField52X(AbstractMT mtMessage, WireTicket wire, bool isNotice)
        {
            if(isNotice)
            {
                ...
                {
                    if(wire.ReceivingAccount.UltimateBeneficiaryType == "BIC" && !string.IsNullOrWhiteSpace(wire.ReceivingAccount.UltimateBeneficiary.BICorABA))
                        mtMessage.addField(GetField52A(wire, true));
                    else
                        mtMessage.addField(GetField52D(wire, true));
                }
        ...
        private static Field52A GetField52A(WireTicket wire, bool isNotice)
        {
            ...
            f52A = new Field52A()
                ...
                .setBIC(wire.SendingAccount.UltimateBeneficiaryType == "BIC" ? wire.SendingAccount.UltimateBeneficiary.BICorABA : string.Empty);
            return f52A;
        }
        ...
        private static Field52D GetField52D(WireTicket wire, bool isNotice)
        {
            ...
            if(isNotice)
            {
                var shouldUseSsi = (!wire.IsFundTransfer && !wire.IsNoticeToFund);

                var isAbaAvailable = shouldUseSsi
                ...
                if(!isAbaAvailable)
                {
                    f52D.setAccount(shouldUseSsi ? !string.IsNullOrWhiteSpace(wire.SSITemplate.FFCNumber)
                            ? wire.SSITemplate.FFCNumber
                            : wire.SSITemplate.UltimateBeneficiaryAccountNumber
                        : !string.IsNullOrWhiteSpace(wire.ReceivingAccount.FFCNumber) ? wire.ReceivingAccount.FFCNumber
            ...
            nameAndAddress = !string.IsNullOrWhiteSpace(wire.SendingAccount.FFCName)
                ? wire.SendingAccount.FFCName
                : !string.IsNullOrWhiteSpace(wire.SendingAccount.UltimateBeneficiaryAccountName)
                    ? wire.SendingAccount.UltimateBeneficiaryAccountName
                    : wire.SendingAccount.UltimateBeneficiary.BankName;

            nameAndAddress += $"\n{wire.FundRegisterAddress}";

            f52D = new Field52D()
                .setAccount(!string.IsNullOrWhiteSpace(wire.SendingAccount.FFCNumber) ? wire.SendingAccount.FFCNumber : wire.SendingAccount.UltimateBeneficiaryAccountNumber)
                .setNameAndAddress(nameAndAddress);
        ...
        private static void SetField56X(AbstractMT mtMessage, WireTicket wire, bool isNotice)
        {
            if(isNotice)
            {
                ...
                if(shouldUseSsi)
                {
                    ...
                    var isBicIntermediarySsiAvailableForNotice = (wire.SSITemplate.IntermediaryType == "BIC" && !string.IsNullOrWhiteSpace(wire.SSITemplate.Intermediary.BICorABA));
                    var isAbaIntermediarySsiAvailableForNotice = (wire.SSITemplate.IntermediaryType == "ABA" && !string.IsNullOrWhiteSpace(wire.SSITemplate.Intermediary.BICorABA));

                    if(isBicIntermediarySsiAvailableForNotice)
                    {
                        mtMessage.addField(new Field56A()
                            .setAccount(wire.SSITemplate.IntermediaryAccountNumber ?? wire.SSITemplate.BeneficiaryAccountNumber)
                            .setBIC(wire.SSITemplate.Intermediary.BICorABA ?? string.Empty));
                        return;
                    }

                    if(isAbaIntermediarySsiAvailableForNotice)
                    {
                        mtMessage.addField(new Field56D()
                            .setAccount(wire.SSITemplate.Intermediary.BICorABA ?? string.Empty)
                            .setNameAndAddress($"{wire.SSITemplate.Intermediary.BankName}\n{wire.SSITemplate.Intermediary.BankAddress}"));
                        return;
                    }

                    var isBicBeneficiarySsiAvailableForNotice = (wire.SSITemplate.BeneficiaryType == "BIC" && !string.IsNullOrWhiteSpace(wire.SSITemplate.Beneficiary.BICorABA));
                    var isAbaBeneficiarySsiAvailableForNotice = (wire.SSITemplate.BeneficiaryType == "ABA" && !string.IsNullOrWhiteSpace(wire.SSITemplate.Beneficiary.BICorABA));


                    if(isBicBeneficiarySsiAvailableForNotice)
                    {
                        mtMessage.addField(new Field56A()
                            .setAccount(wire.SSITemplate.IntermediaryAccountNumber ?? wire.SSITemplate.BeneficiaryAccountNumber)
                            .setBIC(wire.SSITemplate.Beneficiary.BICorABA ?? string.Empty));
                        return;
                    }

                    if(isAbaBeneficiarySsiAvailableForNotice)
                    {
                        mtMessage.addField(new Field56D()
                            .setAccount(wire.SSITemplate.Beneficiary.BICorABA ?? string.Empty)
                            .setNameAndAddress($"{wire.SSITemplate.Beneficiary.BankName}\n{wire.SSITemplate.Beneficiary.BankAddress}"));
                        return;
                    }
                }
                else
                {
                    var isBicIntermediaryReceAvailableForNotice = (wire.ReceivingAccount.IntermediaryType == "BIC" && !string.IsNullOrWhiteSpace(wire.ReceivingAccount.Intermediary.BICorABA));
                    var isAbaIntermediaryReceAvailableForNotice = (wire.ReceivingAccount.IntermediaryType == "ABA" && !string.IsNullOrWhiteSpace(wire.ReceivingAccount.Intermediary.BICorABA));
                    ...
                    if(isAbaIntermediaryReceAvailableForNotice)
                    {
                        mtMessage.addField(new Field56D()
                            .setAccount(wire.ReceivingAccount.Intermediary.BICorABA ?? string.Empty)
                            .setNameAndAddress($"{wire.ReceivingAccount.Intermediary.BankName}\n{wire.ReceivingAccount.Intermediary.BankAddress}"));
                        return;
                    }

                    var isBicBeneficiaryReceAvailableForNotice = (wire.ReceivingAccount.BeneficiaryType == "BIC" && !string.IsNullOrWhiteSpace(wire.ReceivingAccount.Beneficiary.BICorABA));
                    var isAbaBeneficiaryReceAvailableForNotice = (wire.ReceivingAccount.BeneficiaryType == "ABA" && !string.IsNullOrWhiteSpace(wire.ReceivingAccount.Beneficiary.BICorABA));
                    ...
                    if(isAbaBeneficiaryReceAvailableForNotice)
                    {
                        mtMessage.addField(new Field56D()
                            .setAccount(wire.ReceivingAccount.Beneficiary.BICorABA ?? string.Empty)
                            .setNameAndAddress($"{wire.ReceivingAccount.Beneficiary.BankName}\n{wire.ReceivingAccount.Beneficiary.BankAddress}"));
                        return;
                    }
                }
            }
        ...
        private static Field57A GetField57A(WireTicket wire)
        {
            var f57A = new Field57A()
                .setAccount((wire.IsFundTransfer || wire.IsNoticeToFund) ? wire.ReceivingAccount.BeneficiaryAccountNumber : wire.SSITemplate.BeneficiaryAccountNumber)
                .setBIC((wire.IsFundTransfer || wire.IsNoticeToFund) ? wire.ReceivingAccount.BeneficiaryType == "ABA" ? string.Empty :
        ...
        {
            var beneficiaryBicOrAba = (wire.IsFundTransfer || wire.IsNoticeToFund)
        ...
        private static Field58A GetField58A(WireTicket wire)
        {
            var f58A = new Field58A()
                .setAccount((wire.IsFundTransfer || wire.IsNoticeToFund) ? wire.ReceivingAccount.UltimateBeneficiaryAccountNumber : wire.SSITemplate.UltimateBeneficiaryAccountNumber)
                .setBIC((wire.IsFundTransfer || wire.IsNoticeToFund) ? wire.ReceivingAccount.UltimateBeneficiaryType == "ABA" ? string.Empty :
        ...
        {
            var isAbaAvailable = (wire.IsFundTransfer || wire.IsNoticeToFund)
                ? wire.ReceivingAccount.UltimateBeneficiaryType == "ABA" && !string.IsNullOrWhiteSpace(wire.ReceivingAccount.UltimateBeneficiary.BICorABA)
                : wire.SSITemplate.UltimateBeneficiaryType == "ABA" && !string.IsNullOrWhiteSpace(wire.SSITemplate.UltimateBeneficiary.BICorABA);
        ...
        private static Field59 GetField59(WireTicket wire)
        {
            var f59 = new Field59()
                .setAccount((wire.IsFundTransfer || wire.IsNoticeToFund) ? wire.ReceivingAccount.UltimateBeneficiaryAccountNumber : wire.SSITemplate.UltimateBeneficiaryAccountNumber)
                .setNameAndAddress((wire.IsFundTransfer || wire.IsNoticeToFund)
                    ? $"{wire.ReceivingAccount.UltimateBeneficiaryAccountName}\n{wire.FundRegisterAddress}"
                    : $"{wire.SSITemplate.UltimateBeneficiaryAccountName}\n{wire.ReceivingSsiUltimateBeneAccountAddress}");
        ...
        public static Field72 GetField72(WireTicket wire, string messageType)
        {
            ...
            var ffcNumber = (wire.IsFundTransfer || wire.IsNoticeToFund)
