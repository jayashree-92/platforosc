...
BEGIN
				...
				AS
					SELECT onBoardingAccountId,acc.dmaAgreementOnBoardingId,typ.dmaAgreementTypeId,typ.AgreementType,
			...
			CASE WHEN LEN(acc.FFCNumber) >0 THEN acc.FFCNumber ELSE acc.UltimateBeneficiaryAccountNumber  END AS AccountNumber,
				...
				acc.IsExcludedFromTreasuryMarginCheck,CC.CustodianCompanyName,expTyp.AgreementType as MarginExposureType
				...
				LEFT JOIN HM.HMADMIN.onboardingFund obfund  WITH(NOLOCK) ON obfund.FundMapId = F.FundID
				LEFT JOIN HM.HMADMIN.onboardingClient obClient WITH(NOLOCK) ON obClient.dmaClientOnBoardId = obfund.dmaClientOnBoardId  
				LEFT JOIN HM.DBO.hCompanyDetails hCompany WITH(NOLOCK) ON hCompany.CompanyID = obClient.CompanyId   
				...
				LEFT JOIN HM.HMADMIN.dmaAgreementTypes expTyp  WITH(NOLOCK) on acc.MarginExposureTypeID = expTyp.dmaAgreementTypeId 
				...
				LEFT JOIN HM.HMADMIN.dmaOnBoardingAdminChoice OBAC  WITH(NOLOCK) ON OBAC.dmaOnBoardingAdminChoiceId = obfund.FundAdministrator
				LEFT JOIN HM.HMADMIN.custodianCompany CC  WITH(NOLOCK) ON CC.onboardingCustodianCompanyId = obfund.FundCustodian
