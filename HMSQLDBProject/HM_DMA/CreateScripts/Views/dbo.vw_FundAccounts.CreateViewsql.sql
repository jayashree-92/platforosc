...
BEGIN
			...
			CASE WHEN acc.dmaAgreementOnBoardingId >0 THEN	CONCAT(typ.AgreementType,'' between '',LFund.LegalFundName,'' and '',cp.CounterpartyName) ELSE '''' END AS AgreementLongName,  
			CASE WHEN acc.dmaAgreementOnBoardingId >0 THEN	CONCAT(typ.AgreementType,'' - '',LFund.LegalFundName,'' - '',cp.CounterpartyShortCode) ELSE '''' END AS AgreementShortName,  
